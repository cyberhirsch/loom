...
### Instant sound — default voices
Players emit **Note** cables, but a freshly dropped player must make sound with zero patching: each player has a **built-in default voice** (e.g. Melody → marimba, Chords → warm pad, Bass → soft keys, Drums → a kit — mirroring the prototype). Patching the player's Note output to any instrument/plugin bypasses the default voice. No extra nodes required for the common case; full routing freedom when wanted.
...
- **Tauri desktop build:** native hosting via the sidecar — **CLAP first** (cleanest spec, best fit for per-note expression); VST3/AU follow. In-process for v1 with crash-guarded scanning and a blacklist. *(Out-of-process sandboxing and architecture bridging: deferred, §6.12.)*
...
| **M1 — Theory engine hardened** | Extract the player engine as a pure, tested TS library (the product's crown jewel): scales, progressions, Coach scoring, phrase/cadence logic, seeds/determinism, Drums groove layer | Deterministic across browsers; property-tested (always in scale, cadences land); listening-panel pass |
...
| **M3 — Instruments, FX & WAM** 🟡 **FX growing** | Built-in synth/sampler/kit, core FX set (all portable DSP); WAM plugin hosting in the web build; players drive them. **Status: built-in voices (4 synth roles + kit) + reverb + tempo-synced ping-pong delay + limiter, all in the Rust core. Remaining: sampler, EQ/compressor, WAM hosting.** | An ensemble template sounds *good*, not demo-good |
| **M4 — Capture, timeline & modulation** 🟡 **capture + export built** | Capture-to-clip, arrangement view, automation lanes, full modulator node set, macros. **Status: (1) deterministic Standard-MIDI-File export shipped (per-player tracks, GM drums); (2) capture/freeze shipped — a frozen player's take plays verbatim, immune to evolve/regen, persisted with the project (the §5.2 'freeze a region out of the generative flow', working).** Remaining: timeline view, automation lanes. | Generate → capture → arrange → a finished 2-min piece, entirely in Loom |
| **M5 — Launcher, mixer & polish** 🟡 **mixer + templates built** | Session/launcher view, graph-derived mixer, ensemble templates, export/stems, crash recovery. **Status: graph-derived mixer bar shipped (strips ↔ nodes bidirectional); offline WAV bounce shipped (§6.10 — verified ~30× faster than real time); ensemble templates shipped (Ambient garden / Lo-fi trio / Techno engine, §6.11). Remaining: launcher/scenes.** | A performable live set from launcher scenes |
| **M6 — Desktop + native plugins** | **Tauri shell + native sidecar hosting CLAP (then VST3/AU)** — same DSP core native (§8); better audio I/O & filesystem | The same project runs identically in web and desktop; a native plugin loads as a node |
| **M7 — Beta** | Performance tuning, docs, onboarding | Public beta (web + desktop) |
