...
| **M6 — Desktop + native plugins** 🟡 **shell built** | **Tauri shell + native sidecar hosting CLAP (then VST3/AU)** — same DSP core native (§8); better audio I/O & filesystem. **Status: Tauri desktop shell built & launch-verified on Windows (`loom-desktop.exe`, 8.3 MB, real window). Remaining: CLAP sidecar (loom-dsp's C ABI is the intended seam), VST3/AU.** | The same project runs identically in web and desktop; a native plugin loads as a node |
