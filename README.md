...
- **Tension node** — the ensemble's energy curve as a CV source (the music listening to itself, PRD §5.2). Patch cv → density and the loop self-balances: saturated loops breathe out, sparse loops build.
- **Node creation everywhere** — the top bar has a dropdown per category (**+ player · + instrument · + fx · + mod**) covering every node type (singletons grey out when already in the patch), and the canvas right-click menu offers the same catalog plus **copy / cut / paste / delete** on nodes and selections.
